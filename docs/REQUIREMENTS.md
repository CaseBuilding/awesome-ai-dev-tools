...
| # | 问题 | 当前状态 | 建议方向 | 待谁决定 |
|---|------|---------|---------|---------|
| Q1 | 项目未来是否会开源？ | 私有仓库 | 先保持私有，稳定后考虑公开 | CaseBuilding |
| Q2 | 5,000 Stars 门槛是否合适？是否要分层（如 10K+/5K+ 不同区域）？ | 单一门槛 5K | 当前 OK，待收录量 > 500 后重新评估 | CaseBuilding |
| Q3 | 当前 10 个分类是否够用？是否有计划新增/合并？ | 稳定未变动 | 按需扩展——新工具类别出现时再评估 | CaseBuilding |
| Q4 | 是否要支持社区提交项目？ | 不支持 | 不建议（增加审查成本），可考虑 issue 模板接收建议 | CaseBuilding |
| Q5 | README 是否要中英双语？ | 纯中文 | 如目标用户扩展到国际化开发者，可加英文版 | CaseBuilding |
| Q6 | 是否需要测试脚本（如分类准确率测试）？ | 无测试 | 低优先级，分类质量靠人工 review 兜底 | CaseBuilding |
...
| **v1.2** | 质量提升：分类稳定性检查、误报率评估、搜索查询优化 | 📋 计划中 | 2026-06 |
| **v2.0（候选）** | 开源准备：README 英文版、CONTRIBUTING.md、issue 模板 | ❓ 待决策 | 待定 |
