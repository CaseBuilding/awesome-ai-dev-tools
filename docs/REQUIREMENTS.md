...
| 选项 | 你的投入 | Reasonix 的投入 | 适合场景 |
|------|---------|----------------|---------|
| **A) 直接修** | review + 确认（~2 分钟） | 完整排查 + 修复 | 大部分问题 |
| **B) 暂停** | 说"先放着"（~10 秒） | 记录问题到 known issues | 不影响使用的小问题 |
| **C) 自己来** | 按建议手动操作（~5 分钟） | 出方案和步骤 | 你想了解底层逻辑时 |
| **D) 回滚** | 说"回滚"（~10 秒） | 执行回滚操作 | 大问题导致合集不可看 |
