...
| 11 | 🎬 视频处理 / 生成 | 视频剪辑、AI 生成、增强、分析工具 | supervision, moviepy, Wan2.2, Anime4K |
...
│ > 1364 个项目 · 每周日自动更新              │
│ ┌─ 统计（分类明细表）──────────────┐       │
│ │ 🤖 AI 编码助手  47 │ 📄 文档知识管理  59 │   │
│ │ 🔧 Agent 框架  155 │ 🔍 代码分析      94 │   │
│ │ ...                 │                    │   │
│ │ 合计: 1364          │ 更新: 6/7        │   │
│ └────────────────────────────────┘         │
├─ 📑 导航（6 列分组表格）──────────────────────────┤
│ │ 概览 │ AI 工具方向 │ 代码方向 │ 平台方向 │ 基础设施 │ 知识与资源 │
│ │ 关注 │ AI 编码助手 │ 代码分析 │ LLM SDK  │ Web 框架 │ 文档知识管理│
│ │      │ MCP 生态    │ 代码评估 │ AI 网关  │ 数据库   │ 学习资料    │
│ │      │ Agent 框架  │ 开发者工具│ 本地推理 │ DevOps   │             │
│ │      │ 浏览器自动化│         │ 工作流自动化│ 安全  │             │
│ │      │ 音视频      │         │          │ 自托管   │             │
├─ 👁️ 我的关注 ─────────────────────────────┤
...
页面顶部提供 4 列分组表格，分类按方向分组展示。分组规则：

| 分组 | 包含分类 |
|------|---------|
| 概览 | 我的关注（有条件）|
| AI 工具方向 | AI Coding Agent, MCP/工具生态, Agent/工作流, 浏览器自动化, 视频处理/生成 |
| 代码方向 | 代码分析/理解, 代码导图/可视化, 评估/可观测 |
| 平台方向 | LLM 框架/SDK, AI 网关/API 管理, 本地推理 |
...
| 概览 | AI 工具方向 | 代码方向 | 平台方向 | 基础设施 | 知识与资源 |
| --- | --- | --- | --- | --- | --- |
| [👁️ 我的关注] | [🤖 AI 编码助手] | [🔍 代码分析与质量] | [📚 LLM 开发 SDK] | [🌐 Web 框架与服务器] | [📄 文档与知识管理] |
|                | [⛓️ MCP 生态]      | [📏 AI 评估与监控] | [🚪 AI API 管理] | [🗄️ 数据库与存储] | [📖 学习资料] |
|                | [🔧 Agent 开发框架] | [🛠️ 通用开发者工具] | [🧠 本地推理与部署] | [☁️ DevOps 与部署] | |
|                | [🌐 浏览器自动化]     |                     | [⚡ 工作流自动化] | [🔐 安全与渗透] | |
|                | [🎬 音视频与多媒体]   |                     |                    | [🏠 自托管服务] | |
```

锚点名称：分类名转小写，非字母/中文转 `-`，去掉首尾 `-`。
...
### 🥇 #1 — owner/repo ⭐199.6K · 🔤JavaScript
🌏 **中文描述**
📝 英文描述
🔗 [GitHub](url)
...
- 前 3 个分类默认展开，其余折叠
- 精选推荐：每个分类 `confidence=high` 的 Top 5 项目
  - 标题显示 `（Top 5 / 共 N）`，N 为该分类总数
  - 前三名用 🥇🥈🥉 奖牌，后续用纯数字
  - 项目编号在整个分类内连续（精选 #1-#5，全部项目 #6 起）
- 全部项目与精选之间用 `━━━` 长横线分隔
...
层级 2: ## 📊 统计                           ← 统计区（分类明细表）
层级 3: ## 📑 导航                           ← 4 列分组导航表格
层级 4: <details><summary>分类名 <code>36</code></summary>  ← 分类标题（可折叠）
层级 5: ### ⭐ 精选推荐（Top 5 / 共 36）      ← 精选分组标题
层级 6: ### 🥇 #1 — owner/repo ⭐ Stars      ← 项目标题（含编号）
层级 7: 🌏 **中文描述** / 📝 英文描述 / 🔗 [GitHub](url)
```
...
| 字段 | 格式 | 长度限制 | 示例 |
|------|------|---------|------|
| **项目名（精选）** | `### 🥇 #1 — owner/repo ⭐Stars · 🔤语言` | 完整显示 | `### 🥇 #1 — affaan-m/ECC ⭐199.6K · 🔤JavaScript` |
| **项目名（全部）** | `### #6 — owner/repo ⭐Stars · 🔤语言` | 完整显示 | `### #6 — hesreallyhim/awesome-claude-code ⭐45.3K · 🔤Python` |
| **Stars 格式化** | ≥1000 显示 `X.XK`，否则显示整数 | — | `14.7K`、`5000` |
| **语言** | 来自 GitHub API 的 `language` 字段 | — | `TypeScript`, `Python` |
| **语言缺失** | 不显示 `· 🔤` 部分 | — | `### owner/repo ⭐5K` |
| **中文描述** | `🌏 **中文描述...**`（有则显示，无则显示 `*中文描述待补充*`） | 200 字截断 | `🌏 **为 DeepSeek 原生的终端 AI 编程 Agent**` |
...
| 隐藏项目数 | 13 | ≤ 20 | `classified.json` 的 `hidden` 长度 |
...
| Q3 | 当前 10 个分类是否够用？ | ✅ **已扩展至 19 个分类** | 新工具类别出现时再评估 |
...
| **v1.1** | 扩展至 19 个分类；三通道搜索+AI 辅助分类 | ✅ 已完成 | 2026-06 |
| **v1.2** | 中英双语 README + 质量提升：AI 补充缺失中文描述、测试体系建立（57 个测试）、新项目 🆕 标记 | ✅ 已完成 | 2026-06 |
| **v1.3** | **确定性分类锁定 + AI 分类 Skill**：classification-locks.json、锚定样本库、ai-classify-reviewer skill | ✅ 已完成 | 2026-06 |
| **v1.4** | 全量深度 Review + 分类修正 | 🔄 当前迭代 | 2026-06 |
